...
%\paragraph{FragmentedHeap.} The fragmented heap benchmark allocates objects of 300 slots maximum, releases half of them, and re-allocates half of the amount of objects with a minimum size of 600 slots.


\paragraph{LargeObjects.} The LargeObjects benchmark allocates objects larger than 65535 32-bits machine words. The time spent in scavenges is therefore lower than with smaller objects (that are not allocated in the old space at first). Allocating large objects has an impact on the number operations of memory growing and shrinking, which can be lowered by tuning the associated parameters in the VM. In this example, the New-Mem cases have memory segments four times bigger, and four empty segments are available instead of one.
